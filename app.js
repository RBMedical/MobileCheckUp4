const APP_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbzYsOWVHvCcIM0-_OXzIWsz892FFIObmeD4gfYvfP-6xKTXINbVBBQknYnb14vfhwyn9A/exec';
const fields = [
  'HN',
  'ชื่อ นามสกุล',
  'รหัสประจำตัว',
...
async function registerEmployee() {
  ...
  // พิมพ์อัตโนมัติหลังลงทะเบียน — ใช้ Swal เป็น user gesture เพื่อให้ browser อนุญาต print
  if (currentStickers.length) {
    const printResult = await Swal.fire({
      title: 'ลงทะเบียนเรียบร้อย',
      text: 'ต้องการพิมพ์สติกเกอร์เลยไหม?',
      icon: 'success',
      showCancelButton: true,
      confirmButtonText: 'พิมพ์เลย',
      cancelButtonText: 'ไม่พิมพ์ตอนนี้',
      confirmButtonColor: '#087d86',
      cancelButtonColor: '#6c757d',
      timer: 5000,
      timerProgressBar: true
    });
    if (printResult.isConfirmed) window.print();
  }
}
...
function renderStickers(stickers) {
  ...
  preview.innerHTML = stickers.map((item) => {
    // คำนวณ barcode ใหม่: specimenCode(2) + HN(6 หลัก padding)
    const specimenCode = item.specimenCode || getSpecimenCodeFromBarcode(item.barcode, item.HN);
    const hnPadded = String(item.HN || '').trim().padStart(6, '0').slice(-6);
    const barcodeValue = specimenCode
      ? buildBarcodeValue(specimenCode, item.HN)
      : (item.barcode || '');
...
function printStickers() {
  if (!currentStickers.length) {
    return setStatus('ไม่มีสติกเกอร์ที่จะพิมพ์', false);
  }
  window.print();
}
...
function getSpecimenCodeFromBarcode(barcode, hn) {
  ...
  if (!barcodeText || barcodeText.length < 8) return '';
  return barcodeText.slice(0, 2);
}
...
function buildBarcodeValue(specimenCode, hn) {
  // barcode = specimenCode(2 หลัก) + HN padded to 6 digits
  const code = String(specimenCode || '').slice(-2).padStart(2, '0');
...
async function processBarcode() {
  ...
  // check first 2 digits match specimenF
  const specimenCode = $('#specimenF').value.trim();
  const first2 = barcode.slice(0, 2);
  ...
  // barcode = specimenCode(2 หลักแรก) + HN(6 หลักหลัง) = 8 หลัก
  const hnRaw = barcode.slice(2); // 6 หลักหลัง
  const hn = String(parseInt(hnRaw, 10)); // แปลงเป็นตัวเลข (ตัด 0 ข้างหน้า)
  ...
  statusEl.className = 'specimen-barcode-status ok';
  setSpecimenStatus(`บันทึก HN ${hnRaw} — ชื่อ: ${result.patientName || '(ไม่พบในระบบ)'} [${result.debug || ''}]`);
